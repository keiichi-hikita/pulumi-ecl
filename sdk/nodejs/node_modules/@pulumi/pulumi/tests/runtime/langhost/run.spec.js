...
describe("rpc", () => {
    ...
    const cases = {
        ...
        "versions": {
            ...
            expectResourceCount: 3,
            registerResource: (ctx, dryrun, t, name, res, dependencies, custom, protect, parent, provider, propertyDeps, ignoreChanges, version) => {
                switch (name) {
                    case "testResource":
    ...
    for (const casename of Object.keys(cases)) {
        ...
        it(`run test: ${casename} (pwd=${opts.pwd},prog=${opts.program})`, util_1.asyncTest(() => __awaiter(this, void 0, void 0, function* () {
            ...
            for (const dryrun of [true, false]) {
                ...
                (call, callback) => {
                    ...
                    if (opts.invoke) {
                        ...
                        const { failures, ret } = opts.invoke(ctx, req.getTok(), args, version);
                    ...
                    if (req.getType() !== index_1.runtime.rootPulumiStackTypeName || opts.showRootResourceRegistration) {
                        if (opts.registerResource) {
                            ...
                            const propertyDeps = Array.from(req.getPropertydependenciesMap().entries())
                                .reduce((o, [key, value]) => {
                                return Object.assign({}, o, { [key]: value.getUrnsList().sort() });
                            }, {});
                            const version = req.getVersion();
                            const importID = req.getImportid();
                            const { urn, id, props } = opts.registerResource(ctx, dryrun, t, name, res, deps, custom, protect, parent, provider, propertyDeps, ignoreChanges, version, importID);
                            resp.setUrn(urn);
                            resp.setId(id);
                ...
                yield new Promise((resolve, reject) => {
                    monitor.server.tryShutdown((err) => {
                        if (err) {
                            reject(err);
                        }
                        else {
...
function createMockEngine(opts, invokeCallback, readResourceCallback, registerResourceCallback, registerResourceOutputsCallback, logCallback, getRootResourceCallback, setRootResourceCallback, supportsFeatureCallback) {
    ...
    if (!opts.skipRootResourceEndpoints) {
        engineImpl = Object.assign({}, engineImpl, { getRootResource: getRootResourceCallback, setRootResource: setRootResourceCallback });
    }
