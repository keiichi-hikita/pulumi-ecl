...
function prepareResource(label, res, custom, props, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (custom) {
            let resolveValue;
            let resolveIsKnown;
            res.id = new output_1.Output(res, debuggable_1.debuggablePromise(new Promise(resolve => resolveValue = resolve), `resolveID(${label})`), debuggable_1.debuggablePromise(new Promise(resolve => resolveIsKnown = resolve), `resolveIDIsKnown(${label})`), Promise.resolve(false));
            resolveID = (v, isKnown) => {
                resolveValue(v);
...
function listResourceOutputs(typeFilter, stackName) {
    ...
    }).then(({ outputs }) => Object.values(outputs)))
        .map(({ type: typ, outputs }) => {
        return Object.assign({}, outputs, { __pulumiType: typ });
    })
        .filter(typeFilter);
}
